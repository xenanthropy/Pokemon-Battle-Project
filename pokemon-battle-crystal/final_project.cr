...
class PokemonCrystal
	...
	@@playerChoice = ""
	@@opponentChoice = ""
	@@playerStats = [] of Int16 | String
	@@opponentStats = [] of Int16 | String
	@@playerCurrentHP = 1_i16
	@@opponentCurrentHP = 1_i16
	@@winner = ""
	@@IV = 31_i8
	@@LEVEL = 100_i8
	@@EV = 85_i8
	...
	def selectPokemon()
		...
		listOfPokemon = JSON.parse(pokemonFile).as_h
		totalList = listOfPokemon.keys

		moveRegex = /[123456]/
		#Makes sure the user selects a valid input between 1 and 6 
		while(moveRegex.match(selectedPokemon).nil? || selectedPokemon.size != 1)
		#while(selectedPokemon < 1 || selectedPokemon > 6)
			puts "Please select your pokemon by number (1-6): "
			userInput = gets
			exit if userInput.nil?
			
			userInputResult = userInput.presence || "1"
			selectedPokemon = userInputResult.chomp
			if(moveRegex.match(selectedPokemon).nil? || selectedPokemon.size != 1)
			#if(selectedPokemon < 1 || selectedPokemon > 6)
				puts("Please select between 1 and 6!")
			end
		end
		puts "You've selected: " + (totalList[selectedPokemon.to_i - 1]).to_s
		return selectedPokemon.to_i
	...
	def startBattle(playerPokemon)
		...
		@@playerCurrentHP = @@playerStats[1].to_i16
		@@opponentCurrentHP = @@opponentStats[1].to_i16


		displayHealth(playerName, opponentName, @@playerStats[1], @@opponentStats[1])

		if @@playerStats[6].to_i16 >= @@opponentStats[6].to_i16
	...
	def playerAttack()
		...
		if(@@playerCurrentHP > 0)
			...
			moveDict = JSON.parse(pokemonFile).as_h

			moveInfo = moveDict[move].as_a
			#puts moveInfo

			moveType = moveInfo[0]
			moveStatus = moveInfo[1]
			movePower = moveInfo[2].as_i
			moveAccuracy = moveInfo[3].as_i
	...
	def opponentAttack()
		...
		if(@@opponentCurrentHP > 0)
			...
			moveType = moveInfo[0].as_s
			moveStatus = moveInfo[1].as_s
			movePower = moveInfo[2].as_i
			moveAccuracy = moveInfo[3].as_i
	...
	def calculateDamage(moveType, moveStatus, movePower, selection)
		...
		if(selection == "Player")
			...
			if(moveStatus == "Physical")
				attack = @@playerStats[2].to_i16
				defense = @@opponentStats[3].to_i16
				defenseType = @@opponentStats[0].to_s

			elsif(moveStatus == "Special")
				attack = @@playerStats[4].to_i16
				defense = @@opponentStats[5].to_i16
				defenseType = @@opponentStats[0].to_s
			else
				attack = 1
				defense = 1
		...
		elsif(selection == "Opponent")

			if(moveStatus == "Physical")
				attack = @@opponentStats[2].to_i16
				defense = @@playerStats[3].to_i16
				defenseType = @@playerStats[0].to_s

			elsif(moveStatus == "Special")
				attack = @@opponentStats[4].to_i16
				defense = @@playerStats[5].to_i16
				defenseType = @@playerStats[0].to_s
			else
				attack = 1
				defense = 1
	...
	def selectMove(attacker)
		...
		learnedMoves = JSON.parse(pokemonFile).as_h

		if(attacker == "Player")
			moves = learnedMoves[@@playerChoice].as_a
			count = 1
			...
			while(selectedMove < 1 || selectedMove > 4)
				...
				userInputSelection = userInput.presence || "1"
				selectedMove = userInputSelection.chomp.to_i
	...
	def getStats(name)
		...
		pokeType = baseStats[0].as_s
		hp = ((((2 * baseStats[1].as_i + @@IV + (@@EV/4)) * @@LEVEL) / 100) + @@LEVEL + 10).to_i16
		attack = (((((2 * baseStats[2].as_i + @@IV + (@@EV/4)) * @@LEVEL)/100)+5) * @@NATURE).to_i16
		defense = (((((2 * baseStats[3].as_i + @@IV + (@@EV/4)) * @@LEVEL)/100)+5) * @@NATURE).to_i16
		spattack = (((((2 * baseStats[4].as_i + @@IV + (@@EV/4)) * @@LEVEL)/100)+5) * @@NATURE).to_i16
		spdefense = (((((2 * baseStats[5].as_i + @@IV + (@@EV/4)) * @@LEVEL)/100)+5) * @@NATURE).to_i16
		speed = (((((2 * baseStats[6].as_i + @@IV + (@@EV/4)) * @@LEVEL)/100)+5) * @@NATURE).to_i16
